...
对输入特征 $x$，输出位置 $\mathbf{p}_0$：
\begin{equation}
y(\mathbf{p}_0)=\sum_{k=1}^{K} w_k\cdot x\left(\mathbf{p}_0+\mathbf{p}_k+\Delta\mathbf{p}_k\right),
\label{eq:dcn}
\end{equation}
其中 $\mathbf{p}_k$ 为规则网格偏移，$\Delta\mathbf{p}_k$ 为可学习偏移。

\subsubsection{解耦偏移预测（避免几何-语义前向耦合）}
偏移由轻量网络 $g(\cdot)$ 从局部特征预测：
\begin{equation}
\Delta\mathbf{P}=g(F)\in\mathbb{R}^{K\times 2}.
\label{eq:offset}
\end{equation}
关键点：$g(\cdot)$ 不直接使用当前中心线/切线作为输入，从而避免“预测几何 $\rightarrow$ 引导采样 $\rightarrow$ 影响几何”的前向耦合链路。

\subsubsection{正交散布约束：仅约束法向覆盖，保留切向自由度}
我们不对偏移的切向方向作硬对齐，而是约束其在法向方向上的\textbf{散布}满足椎体块状形状先验。定义投影：
\begin{equation}
d_k=\Delta\mathbf{p}_k\cdot \hat{\mathbf{n}}_i,\quad s_k=\Delta\mathbf{p}_k\cdot \hat{\mathbf{t}}_i,
\end{equation}
并以方差刻画散布幅度：
\begin{equation}
...
\end{equation}
结合无量纲形状比 $r_i$（式\ref{eq:ratio}），我们约束散布比值：
\begin{equation}
...
\label{eq:lortho}
\end{equation}
其中 $\beta$ 为可学习标量或常数。该设计同时实现：
\begin{itemize}
    \item \textbf{形状对齐}：法向散布与椎体宽高比一致，避免采样退化为细长一维；
    \item \textbf{尺度鲁棒}：约束量为无量纲比值，抵抗缩放/分辨率变化；
    \item \textbf{环路抑制}：几何量采用 $\mathrm{sg}(\cdot)$ 派生，降低梯度循环风险。
\end{itemize}

\subsubsection{弱防漂移正则}
为避免偏移无界增大引起采样漂移，加入弱正则：
\begin{equation}
